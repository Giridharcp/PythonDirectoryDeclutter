...
def checkStop():
    return app.yesNoBox("Confirm Exit", "Do you wanna gooo?")
def press(button):
    if button == "<Delete>":
       app.clearAllEntries(callFunction=False)
        ...
        if path=='':
            app.errorBox("No Directory selected", "Please select a directory!!! ", parent=None)
        else:

            os.chdir(path)
            directories={
            "Android_app":['.apk'],
            "archive":['.7z','.ace','.dd','.b2z','.crx','.dd','.deb','.gz','.jar','.gzip','.jar','.mht','.r01','.rar','.rpm','.sit','.sitx','.snb','.tar','.tar.gz','.tgz','.zip','.zipx'],
            ...
            "Misc_files":['.8bi','.bak','.bbl','.bfc','.bib','.bibtex','.cff','.cmf','.crdownload','.cue','.dao','.dbx','.dem','.dic','.enl','.ens','.enw','.epc','.flt','.fnt','.fon','.gam','.gho','.gpx','.grp','.hex','.hqx','.ics','.idx','.img','.iso','.jad','.kml','.kmz','.map','.marc','.mdf','.mim','.mrc','.msi','.mtb','.nes','.ori','.otf','.part','.pes','.plugin','.ps','.qxp','.rem','.ris','.rom','.sav','.t65','.tec','.tmp','.toast','.torrent','.ttf','.url','.uue','.vcd','.xll'],
            ...
            if __name__ == "__main__": 
                organize_junk() 
            app.infoBox('Declutter Files','Files organized into directories',parent=None)

app=gui("File Organizer", "500x100")
x=app.addDirectoryEntry("f1")
app.enableEnter(press)
app.bindKey("<Delete>", press)
app.bindKey("<Escape>",press)
app.addButtons(["De-Clutter","Exit"], press)
app.go()
